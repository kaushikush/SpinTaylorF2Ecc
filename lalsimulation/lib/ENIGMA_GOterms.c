...
static COMPLEX16 hGO_3_m_3(REAL8 mass,REAL8 Nu,REAL8 r,REAL8 rDOT,REAL8 PhiDOT,UINT4 vpnorder, REAL8 S1z, REAL8 S2z, REAL8 x){
    ...
    if(vpnorder == 1){
     ...
     74954880*delta*pow(mass,3)*(Complex(0,22)*mass*PhiDOT*r + Complex(0,59)*pow(PhiDOT,3)*pow(r,4) + 8*mass*rDOT + 66*pow(PhiDOT,2)*pow(r,3)*rDOT + 
        Complex(0,24)*PhiDOT*pow(r,2)*pow(rDOT,2) - 4*r*pow(rDOT,3))*log(r/r0))/(2.4216192e7*sqrt(210)*pow(r,4))) + /* Henry et al. QC spinning hereditary terms */ ((9*sqrt(0.04285714285714286)*pow(x,4.5)*
     (Complex(0,-5)*M_PI*((-4 - 4*delta + 19*Nu + 5*delta*Nu)*S1z + 
